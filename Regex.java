...
//	  Pattern p = Pattern.compile("([^\\d]+)(\\d+)( - )([^\\s]+) \\(([^\\s]+)-([^\\s]+)\\)");
	private static final String CURSO_PATTERN_STRING = 
			  "(" +											// $1 total match
				...
				"(" + CURSO_NUMERO + ")" +							// $3 C�digo do Curso
				"(" + CURSO_SEPARATOR + ")" +							// $4 Hifen
				"(" + CURSO_NOME + ")" +							// $5 Nome do curso
				CURSO_INICIO_PARENTESES + 							//    Antes do per�odo
				"(" + CURSO_TIPO_PERIODO + ")" +						// $6 Tipo do Per�odo
				HIFEN +										//    Hifen
				"(" + CURSO_CAMPUS + ")" +							// $7 Campus do Curso
				CURSO_FIM_PARENTESES +								//    Par�nteses direito
				WHITESPACES +
				CURSO_VAGAS_STR + 
				WHITESPACES +
				"(" + CURSO_NUM_VAGAS + ")" +							// $8 N�mero de vagas
				WHITESPACES + 
			  ")";
	...
	private static final String CANDIDATO_NOME = "[^\\s{2}]+"; // at� encontrar dois espa�os
	private static final String CANDIDATO_IS_COTISTA = "[(SIM)(N�O)]";
	private static final String CANDIDATO_NOTA = "[^\\w]+"; // qualquer coisa menos espa�os 
	private static final String CANDIDATO_TOTAL_PARCIAL = "[^\\w]+"; // qualquer coisa menos espa�os;
	private static final String CANDIDATO_ESCORE_FINAL = "[^\\w]+"; // qualquer coisa menos espa�os;;
	private static final String CANDIDATO_CLASSIFICACAO = "[^\\w]+"; // qualquer coisa menos espa�os;;
	private static final String CANDIDATO_SITUACAO = "[^\\w]+"; // qualquer coisa menos espa�os;;
	
	  
	  private static final String CANDIDATO_PATTERN_STRING =
			  "(" +															
				"(" + CANDIDATO_INSCRICAO + ")" +						// $1 N�mero de inscri��o
				WHITESPACES +
				"(" + CANDIDATO_NOME + ")" +							// $2 Nome do candidato
				WHITESPACES  +									
				"(" + CANDIDATO_IS_COTISTA + ")" +						// $3 Se candidato � cotista
				WHITESPACES +														
				"(" + CANDIDATO_NOTA + ")" +							// $4 Nota (Conhecimentos Gerais)
				WHITESPACES +										
				"(" + CANDIDATO_TOTAL_PARCIAL + ")" +						// $5 Nota Total P1
				WHITESPACES +
				"(" + CANDIDATO_NOTA + ")" +							// $6 Nota (Reda��o)
				WHITESPACES + 
				"(" + CANDIDATO_NOTA + ")" +							// $7 Nota (Portugu�s)
				WHITESPACES + 
				...
				WHITESPACES +
				"(" + CANDIDATO_CLASSIFICACAO + ")" +					// $11 Classifica��o (no curso)
				WHITESPACES +
				"(" + CANDIDATO_SITUACAO + ")" + 						// $12 Situa��o do Candidato
			  ")";
	  	  
	  
//	  15482-4  ALANA CAROLINA GOMES               SIM     59,0   59,0    90 45,0  6,0I 141,0   29,0  35,0  64,0   264,0   43 AP-COTAS
//	  36538-3  ALEX YUTAKA SARKHEIL               N�O     36,5   36,5    NA 31,5  0,0I   --    30,0  14,5  44,5     --    -- RP      
